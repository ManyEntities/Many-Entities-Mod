...
public class FireMagicGeneratorContainer extends Container {
	final TileEntityFireMagicGenerator tile_entity;
	private int last_time, last_time_max;

	public FireMagicGeneratorContainer(final InventoryPlayer inventory_player, final TileEntityFireMagicGenerator tile_entity) {
	...
	public ItemStack transferStackInSlot(EntityPlayer player, int slotID) {
		...
		if (slot != null && slot.getHasStack()) {
			...
			if (slotID < tile_entity.getSizeInventory()) {
				...
				if (FireMagicGeneratorRecipes.getRecipe(itemstack1) == null)
				     return null;
				if (!this.mergeItemStack(itemstack1, 1, tile_entity.getSizeInventory() - 1, false))
					return null;
			}
