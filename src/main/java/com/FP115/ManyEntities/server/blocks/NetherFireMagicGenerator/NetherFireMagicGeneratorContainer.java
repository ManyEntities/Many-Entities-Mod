...
public class NetherFireMagicGeneratorContainer extends Container {
	final TileEntityNetherFireMagicGenerator tile_entity;
	private int last_time, last_time_max;

	public NetherFireMagicGeneratorContainer(final InventoryPlayer inventory_player, final TileEntityNetherFireMagicGenerator tile_entity) {
		tile_entity.openInventory();
		this.addSlotToContainer(new Slot(tile_entity, 3, 115, 18){
			@Override
		...
		this.addSlotToContainer(new Slot(tile_entity, 4, 115, 64){
			@Override
			public boolean isItemValid(ItemStack itemStack) {
				return false;
			}
		...
		this.addSlotToContainer(new Slot(tile_entity, 1, 115, 41){
			@Override
			public boolean isItemValid(ItemStack itemStack) {
				return false;
			}
	...
	public ItemStack transferStackInSlot(EntityPlayer player, int slotID) {
		...
		if (slot != null && slot.getHasStack()) {
			...
			if (slotID < tile_entity.getSizeInventory()) {
				...
				if (NetherFireMagicGeneratorRecipes.getRecipe(itemstack1) == null)
				     return null;
				if (!this.mergeItemStack(itemstack1, 3, tile_entity.getSizeInventory() - 1, false))
					return null;
			}
