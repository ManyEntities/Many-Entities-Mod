...
public class ObsidianFireMagicGeneratorGui extends GuiContainer {
	final InventoryPlayer inventory_player;
	final TileEntityObsidianFireMagicGenerator tile_entity;

	private static final ResourceLocation texture = new ResourceLocation("manyentities:textures/gui/obsidianfiremagicgenerator.png");

	public ObsidianFireMagicGeneratorGui(final InventoryPlayer inventory_player, final TileEntityObsidianFireMagicGenerator tile_entity) {
		super(new ObsidianFireMagicGeneratorContainer(inventory_player, tile_entity));
	...
	public void drawGuiContainerBackgroundLayer(float parTick, int mouseX, int mouseY) {
		...
		if (tile_entity.time_max > 0) {
			this.drawTexturedModalRect(this.guiLeft + 49, this.guiTop + 24, 0, 166, 83 - (int)((float)(tile_entity.time / (float)tile_entity.time_max) * 84F), 51);
		}
		if(tile_entity.getStackInSlot(2) !=null){
			this.drawTexturedModalRect(this.guiLeft + 67, this.guiTop + 24, 83, 166, 47, 51);
		}
	}
