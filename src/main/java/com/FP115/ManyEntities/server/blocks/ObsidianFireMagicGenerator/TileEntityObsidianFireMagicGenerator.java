...
public class TileEntityObsidianFireMagicGenerator extends TileEntity implements ISidedInventory {
	Random rand = new Random();
	private final ItemStack[] inventory = new ItemStack[4];
	private static final int[]
			topSlots = new int[] {0},
			sideSlots = new int[] {1, 2, 3};
	private ObsidianFireMagicGeneratorRecipes.Recipe recipe = null;
	private String custom_name;
	public int time = 0, time_max = 0;

	public TileEntityObsidianFireMagicGenerator() {}
	...
	public void updateEntity() {
		...
		if (time == 0) {
			...
			if (recipe != null && trash == null ) {
				...
				recipe = ObsidianFireMagicGeneratorRecipes.getRecipe(input);
			...
			if(output1 == null) {
				if(trash==null){
					...
					if(recipe==null)recipe = ObsidianFireMagicGeneratorRecipes.getRecipe(input);
					if(output2 != null && output2.isItemEqual(recipe.output2) && ItemStack.areItemStackTagsEqual(output2, recipe.output2) && recipe.output2.stackSize + output2.stackSize <= 64){
						--time;
					}
				}
